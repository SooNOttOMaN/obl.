...
client.channels.get(client.ayarlar.kayıt).send(`<a:loading:738313832254144552> **| \`${req.user.username}#${req.user.discriminator}\` Adlı Kullanıcı \`${sistem.id}\` ID'sine Sahip \`${sistem.username}#${sistem.discriminator}\` Adlı Botu İle Başvuru Yaptı!**`)

if (client.users.has(req.user.id) === true) {
  client.users.get(req.user.id).send(`<a:loading:738313832254144552> **| \`${sistem.username}#${sistem.discriminator}\` Adlı Botunuz Başarıyla Sistemimize Eklendi! Şimdi, Tek Yapmanız Gereken Şey Yetkililerin Botunu Kontrol Etmesini Beklemek! Bol Şans :tada:**`)
}
...
client.channels.get(client.ayarlar.kayıt).send(`<a:Gvenli:731878935272488993> **| \`${req.user.username}#${req.user.discriminator}\` Adlı Kullanıcı \`${sistem.id}\` ID'sine Sahip \`${sistem.username}#${sistem.discriminator}\` Adlı Botunun Başvurusunu/Profilini Düzenlendi!**`)

if (client.users.has(req.user.id) === true) {
client.users.get(req.user.id).send(`<a:Gvenli:731878935272488993> **| \`${sistem.username}#${sistem.discriminator}\` Adlı Botunuzun Profili/Başvurusu Başarıyla Düzenlendi!**`)
}
...
client.channels.get('738082750657069238').send(`⚠️ **| \`${req.user.username}#${req.user.discriminator}\` Adlı Kullanıcı \`${db.fetch(`botlar.${req.params.botID}.isim`)}\` Adlı Botu Raporladı! \n📝 | Sebep: \`${ayar['mesaj-1']}\`**`)
}
if(ayar['mesaj-2']) {
...
client.channels.get(client.ayarlar.kayıt).send(`<a:tik3:731401344266534915> **| \`${req.user.username}#${req.user.discriminator}\` Adlı Yetkili Tarafından \`${db.fetch(`botlar.${id}.sahip`)}\` Adlı Kullanıcının \`${db.fetch(`botlar.${id}.id`)}\` ID'sine Sahip \`${db.fetch(`botlar.${id}.isim`)}\` Adlı Botu Onaylandı!\n🔺 |  Sahip: <@${db.fetch(`botlar.${id}.sahipid`)}>\n🔺 |  Bot: <@${db.fetch(`botlar.${id}.id`)}>**`)

if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
client.users.get(db.fetch(`botlar.${id}.sahipid`)).send(`<a:tik3:731401344266534915> **| \`${db.fetch(`botlar.${id}.isim`)}\` Adlı Botunuz Onaylandı!\n🔑 **|** İşte Botunun Sitedeki Linki: \n📝 **|** https://ottoman-botlist.glitch.me/bot/${db.fetch(`botlar.${id}.id`)}**`)
}
...
client.channels.get(client.ayarlar.kayıt).send(`<a:yukleniyor:731878990738096230> **| \`${req.user.username}#${req.user.discriminator}\` Adlı Yetkili Tarafından \`${db.fetch(`botlar.${id}.sahip`)}\` Adlı Kullanıcının \`${db.fetch(`botlar.${id}.id`)}\` ID'sine Sahip \`${db.fetch(`botlar.${id}.isim`)}\` Adlı Botu Bekleme Moduna Alındı!**`)

if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
client.users.get(db.fetch(`botlar.${id}.sahipid`)).send(`<a:yukleniyor:731878990738096230> **| \`${db.fetch(`botlar.${id}.isim`)}\` Adlı Botunuz Beklemeye/İncelenmeye Alındı!**`)
}
...
app.post("/botyonetici/reddet/:botID", checkAuth, (req, res) => {
  ...
  client.channels.get(client.ayarlar.kayıt).send(`<a:red3:731401326235353179> **| \`${req.user.username}#${req.user.discriminator}\` Adlı Yetkili Tarafından \`${db.fetch(`botlar.${id}.sahip`)}\` Adlı Kullanıcının \`${db.fetch(`botlar.${id}.id`)}\` ID'sine Sahip \`${db.fetch(`botlar.${id}.isim`)}\` Adlı Botu \`${req.body['red-sebep']}\` Sebebi İle Reddedildi!**`)
  
  if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
  client.users.get(db.fetch(`botlar.${id}.sahipid`)).send(`<a:red3:731401326235353179> **| \`${db.fetch(`botlar.${id}.isim`)}\` Adlı Botunuz \`${req.body['red-sebep']}\` Sebebi İle Reddedildi!\n🔔 | Hataları Giderdikten  Tekrar Başvuru Yapabilirsin!**`)
  }
